...
export async function handleIdeaGenerationFallback(
  ...
  const opts = {
    ...
    retryDelay: 1000,
    enableCaching: true,
    strategy: ["RETRY_WITH_SIMPLE_PROMPT", "RETURN_TEMPLATE_RESPONSE"] as FallbackStrategy[],
    ...options
  };
 ...
 * エラーの重要度を判定
 */
export function classifyErrorSeverity(error: Error): "LOW" | "MEDIUM" | "HIGH" | "CRITICAL" {
 ...
 */
export function sanitizeErrorForLogging(error: Error, includeStack: boolean = false): object {
  const sanitized: any = {
    name: error.name,
