...
describe("SafeJsonParser", () => {
  describe("safeJsonParse", () => {
    ...
    it("should sanitize dangerous keys in parsed JSON", () => {
      const jsonWithDangerousKeys =
        '{"__proto__": {"polluted": true}, "constructor": {"dangerous": true}, "normal": "safe"}';
    ...
    it("should provide meaningful error messages", () => {
      try {
        ...
        expect((error as SafeJsonParseError).parseErrorType).toBe(
          "SYNTAX_ERROR",
        );
    ...
    it("should handle options parameter", () => {
      const largeJson = JSON.stringify({ data: "A".repeat(2000000) }); // 2MB of data

      // Should fail with default limits (1MB)
      expect(() => safeJsonParse(largeJson)).toThrow();

      // Should pass with custom limits
      const result = safeJsonParse(largeJson, { maxSize: 3000000 }); // 3MB limit
      expect(result.data).toBe("A".repeat(2000000));
    });
  });
