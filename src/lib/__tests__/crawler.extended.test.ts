...
describe("Crawler Extended Tests", () => {
  ...
  describe("getEventFilters functionality", () => {
    it("should correctly filter enabled events", async () => {
      ...
      expect(mockLogger.info).toHaveBeenCalledWith("Event filter: event1,event3");
    });
    ...
    it("should handle malformed crawledEvents.json", async () => {
      ...
      await expect(crawlEthGlobalShowcase()).rejects.toThrow();
    });
  ...
  describe("extractProjectDetails edge cases", () => {
    ...
    it("should handle projects with missing elements", async () => {
      const finalistImageUrl = "https://ethglobal.b-cdn.net/organizations/xdat5/square-logo/default.png";
      const showcaseHtml = `
        <div class="block border-2 border-black rounded overflow-hidden relative" href="/project1">
          <img src="${finalistImageUrl}" />
          <!-- Missing h2, p, and hackathon elements -->
        </div>
      `;
    ...
    it("should handle projects with complex HTML in descriptions", async () => {
      ...
      expect(result[0].description).toBe("Description with bold and italic text");
  ...
  describe("fetchProjectDetailPage edge cases", () => {
    it("should handle projects with missing source code link", async () => {
      ...
      mockAxios.get
        .mockResolvedValueOnce({ data: showcaseHtml })
        .mockResolvedValueOnce({ data: "No results found..." })
        .mockResolvedValueOnce({ data: projectDetailHtml });
    ...
    it("should handle malformed HTML in project detail page", async () => {
      ...
      mockAxios.get
        .mockResolvedValueOnce({ data: showcaseHtml })
        .mockResolvedValueOnce({ data: "No results found..." })
        .mockResolvedValueOnce({ data: malformedHtml });
  ...
  describe("Pagination and termination", () => {
    it("should handle multiple pages correctly", async () => {
      ...
      `;
      const page2Html = `
        <div class="block border-2 border-black rounded overflow-hidden relative" href="/project2">
          <img src="${finalistImageUrl}" />
          <h2>Project 2</h2>
        </div>
      `;
      ...
      mockAxios.get
        ...
        .mockResolvedValueOnce({ data: noResultsHtml })
        .mockResolvedValue({ data: "<html></html>" }); // Project detail pages
    ...
    it("should handle immediate no results scenario", async () => {
      ...
      expect(result).toHaveLength(0);
      expect(mockLogger.info).toHaveBeenCalledWith("No more results found. Stopping crawl.");
    });
  ...
  describe("Logging and monitoring", () => {
    it("should log all major steps", async () => {
      ...
      expect(mockLogger.info).toHaveBeenCalledWith(expect.stringContaining("Event filter:"));
      expect(mockLogger.info).toHaveBeenCalledWith("Crawling page 1...");
      expect(mockLogger.info).toHaveBeenCalledWith(expect.stringContaining("Fetching page from"));
      expect(mockLogger.info).toHaveBeenCalledWith("No more results found. Stopping crawl.");
      expect(mockLogger.info).toHaveBeenCalledWith("Crawling complete and projects saved.");
    });
    ...
    it("should log project detail page fetching", async () => {
      ...
      expect(mockLogger.info).toHaveBeenCalledWith(
        expect.stringContaining("Fetching details from https://ethglobal.com/project1")
      );
    });
  });
});