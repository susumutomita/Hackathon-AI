...
describe("PromptSecurityGuard", () => {
  describe("sanitizePromptInput", () => {
    it("should allow normal input", () => {
      const normalInput =
        "DeFiプロトコルのプライズです。Ethereumベースの革新的なソリューションを求めています。";
    ...
    it("should handle edge cases safely", () => {
      ...
      expect(() => sanitizePromptInput("   ")).toThrow(PromptSecurityError);
      expect(() => sanitizePromptInput(null as any)).toThrow(
        PromptSecurityError,
      );
      expect(() => sanitizePromptInput(undefined as any)).toThrow(
        PromptSecurityError,
      );
    });
    ...
    it("should normalize and limit whitespace", () => {
      const inputWithExcessiveWhitespace =
  ...
  describe("buildSecurePrompt", () => {
    it("should create a secure prompt template", () => {
      ...
      expect(result).toContain(
        "IMPORTANT: The following is user-provided content",
      );
    });
    ...
    it("should validate template has user input placeholders", () => {
      ...
      expect(() =>
        buildSecurePrompt(templateWithoutPlaceholder, userInput),
      ).toThrow();
    });
  });
  ...
  describe("PromptSecurityError", () => {
    it("should create error with proper properties", () => {
      const error = new PromptSecurityError(
        "セキュリティエラー",
        "INJECTION_DETECTED",
      );
