...
describe("LLMResponseValidator", () => {
  describe("validateLLMResponse", () => {
    ...
    it("should reject empty or null responses", () => {
      ...
      expect(() => validateLLMResponse("   ")).toThrow(
        LLMResponseValidationError,
      );
      expect(() => validateLLMResponse(null as any)).toThrow(
        LLMResponseValidationError,
      );
      expect(() => validateLLMResponse(undefined as any)).toThrow(
        LLMResponseValidationError,
      );
    });
    ...
    it("should reject responses exceeding maximum length", () => {
      ...
      expect(() => validateLLMResponse(longResponse)).toThrow(
        LLMResponseValidationError,
      );
    });
    ...
    it("should sanitize potentially dangerous script tags", () => {
      const responseWithScript =
        "有効なアイデア<script>alert('xss')</script>です";
    ...
    it("should detect and reject prompt injection attempts", () => {
      ...
      expect(() => validateLLMResponse(promptInjection)).toThrow(
        LLMResponseValidationError,
      );
    });
    ...
    it("should reject responses with suspicious system commands", () => {
      ...
      expect(() => validateLLMResponse(maliciousResponse)).toThrow(
        LLMResponseValidationError,
      );
    });
    ...
    it("should reject responses with excessive repetition", () => {
      ...
      expect(() => validateLLMResponse(repetitiveResponse)).toThrow(
        LLMResponseValidationError,
      );
    });
    ...
    it("should remove dangerous attributes from HTML", () => {
      const responseWithDangerousAttrs = `
        <p style="display:none" class="dangerous">テスト</p>
        <h1 id="myheader" data-test="value">タイトル</h1>
      `;
